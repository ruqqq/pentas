# Papan Tracker Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `papan`, the single-user agent inbox tracker that feeds `dalang`. Server-rendered HTML + HTMX + SQLite, exposed over a small REST API plus an SSE event stream. Conforms to the tracker contract from `docs/superpowers/specs/2026-04-29-dalang-orchestrator-design.md` §11 and implements the full spec at `docs/superpowers/specs/2026-04-29-papan-tracker-design.md`.
...
**Repository:** monorepo skeleton (Phase 0) is shared with `dalang`. After Phase 0 the repo has `packages/papan/` and `packages/dalang/` (stubbed). The dalang plan starts from this state.
...
- Create: `packages/papan/package.json`
- Create: `packages/papan/tsconfig.json`
- Create: `packages/papan/src/index.ts`
- Create: `packages/dalang/package.json`
- Create: `packages/dalang/tsconfig.json`
- Create: `packages/dalang/src/index.ts`

- [ ] **Step 1: Papan `package.json`**
...
- [ ] **Step 3: Papan stub `src/index.ts`**

```typescript
console.log("papan stub");
```
...
Run: `bun run --filter=@pentas/papan start`
Expected: `papan stub`

Run: `bun run --filter=@pentas/dalang start`
...
```bash
git add packages/
git commit -m "chore: add papan and dalang package stubs"
```

---

## Phase 1 — Papan Foundation

### Task 1.1: ULID and identifier sequence

**Files:**

- Create: `packages/papan/src/lib/ids.ts`
- Create: `packages/papan/tests/lib/ids.test.ts`

- [ ] **Step 1: Write failing test** at `packages/papan/tests/lib/ids.test.ts`
...
Run: `bun test packages/papan/tests/lib/ids.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Implement** at `packages/papan/src/lib/ids.ts`
...
```bash
git add packages/papan/src/lib/ids.ts packages/papan/tests/lib/ids.test.ts
git commit -m "feat(papan): add ULID and identifier formatter"
```
...
- Create: `packages/papan/src/db/schema.sql`
- Create: `packages/papan/src/db/migrations.ts`
- Create: `packages/papan/tests/db/migrations.test.ts`
...
- [ ] **Step 2: Write failing test** at `packages/papan/tests/db/migrations.test.ts`
...
Run: `bun test packages/papan/tests/db/migrations.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 4: Implement** at `packages/papan/src/db/migrations.ts`
...
```bash
git add packages/papan/src/db packages/papan/tests/db
git commit -m "feat(papan): add SQLite schema and migration runner"
```
...
- Create: `packages/papan/src/db/seq.ts`
- Create: `packages/papan/tests/db/seq.test.ts`
...
```bash
git add packages/papan/src/db/seq.ts packages/papan/tests/db/seq.test.ts
git commit -m "feat(papan): add identifier sequence allocator"
```
...
- Create: `packages/papan/src/domain/issue.ts`
- Create: `packages/papan/tests/domain/issue.test.ts`
...
```bash
git add packages/papan/src/domain/issue.ts packages/papan/tests/domain/issue.test.ts
git commit -m "feat(papan): add issue state machine and NormalizedIssue type"
```
...
- Create: `packages/papan/src/lib/markdown.ts`
- Create: `packages/papan/tests/lib/markdown.test.ts`
...
```bash
git add packages/papan/src/lib/markdown.ts packages/papan/tests/lib/markdown.test.ts
git commit -m "feat(papan): add sanitized markdown renderer"
```
...
- Create: `packages/papan/src/lib/linear-url.ts`
- Create: `packages/papan/tests/lib/linear-url.test.ts`
...
```bash
git add packages/papan/src/lib/linear-url.ts packages/papan/tests/lib/linear-url.test.ts
git commit -m "feat(papan): add Linear URL parser"
```
...
- Create: `packages/papan/src/db/repo/issues.ts`
- Create: `packages/papan/tests/db/repo/issues.test.ts`
...
Run: `bun test packages/papan/tests/db/repo/issues.test.ts`
Expected: PASS, 7 tests.

- [ ] **Step 5: Commit**

```bash
git add packages/papan/src/db/repo packages/papan/tests/db/repo
git commit -m "feat(papan): add issues repository with CRUD and pagination"
```
...
- Create: `packages/papan/src/db/repo/comments.ts`
- Create: `packages/papan/src/db/repo/history.ts`
- Create: `packages/papan/tests/db/repo/comments.test.ts`
- Create: `packages/papan/tests/db/repo/history.test.ts`
...
```bash
git add packages/papan/src/db/repo/comments.ts packages/papan/src/db/repo/history.ts \
  packages/papan/src/domain/comment.ts packages/papan/src/domain/history.ts \
  packages/papan/tests/db/repo/comments.test.ts packages/papan/tests/db/repo/history.test.ts
git commit -m "feat(papan): add comments and history repositories"
```
...
- Create: `packages/papan/src/lib/sse.ts`
- Create: `packages/papan/tests/lib/sse.test.ts`
...
```bash
git add packages/papan/src/lib/sse.ts packages/papan/tests/lib/sse.test.ts
git commit -m "feat(papan): add in-memory SSE event bus"
```
...
- Create: `packages/papan/src/api/auth.ts`
- Create: `packages/papan/src/api/server.ts`
- Create: `packages/papan/tests/api/auth.test.ts`
...
```bash
git add packages/papan/src/api/auth.ts packages/papan/src/api/server.ts packages/papan/tests/api/auth.test.ts
git commit -m "feat(papan): add server scaffolding and bearer-token middleware"
```
...
- Create: `packages/papan/src/api/routes/issues-list.ts`
- Create: `packages/papan/tests/api/issues-list.test.ts`
...
```bash
git add packages/papan/src/api/routes/issues-list.ts packages/papan/tests/api/issues-list.test.ts
git commit -m "feat(papan): add GET /api/v1/issues route"
```
...
- Create: `packages/papan/src/api/routes/issues-by-ids.ts`
- Create: `packages/papan/src/api/routes/issues-detail.ts`
- Create: `packages/papan/tests/api/issues-detail.test.ts`
...
```bash
git add packages/papan/src/api/routes packages/papan/tests/api/issues-detail.test.ts
git commit -m "feat(papan): add issue by-ids and detail routes"
```
...
- Create: `packages/papan/src/api/routes/issues-create.ts`
- Create: `packages/papan/tests/api/issues-create.test.ts`
...
```bash
git add packages/papan/src/api/routes/issues-create.ts packages/papan/tests/api/issues-create.test.ts
git commit -m "feat(papan): add POST /api/v1/issues route"
```
...
- Create: `packages/papan/src/api/routes/issues-update.ts`
- Create: `packages/papan/src/api/routes/issues-delete.ts`
- Create: `packages/papan/tests/api/issues-update.test.ts`
...
```bash
git add packages/papan/src/api/routes/issues-update.ts packages/papan/src/api/routes/issues-delete.ts packages/papan/tests/api/issues-update.test.ts
git commit -m "feat(papan): add PATCH and DELETE issue routes"
```
...
- Create: `packages/papan/src/api/routes/comments.ts`
- Create: `packages/papan/src/api/routes/history.ts`
- Create: `packages/papan/tests/api/comments.test.ts`
...
```bash
git add packages/papan/src/api/routes/comments.ts packages/papan/src/api/routes/history.ts packages/papan/tests/api/comments.test.ts
git commit -m "feat(papan): add comments and history routes"
```
...
- Create: `packages/papan/src/api/routes/events.ts`
- Create: `packages/papan/tests/api/events.test.ts`
...
```bash
git add packages/papan/src/api/routes/events.ts packages/papan/tests/api/events.test.ts
git commit -m "feat(papan): add SSE events route"
```
...
- Create: `packages/papan/src/ui/public/style.css` (placeholder)
- Create: `packages/papan/src/ui/public/htmx.min.js` (vendored)
- Create: `packages/papan/src/ui/public/sse.js` (vendored)
- Create: `packages/papan/src/ui/static.ts`
...
```bash
mkdir -p packages/papan/src/ui/public
curl -L -o packages/papan/src/ui/public/htmx.min.js https://unpkg.com/htmx.org@2.0.3/dist/htmx.min.js
curl -L -o packages/papan/src/ui/public/sse.js https://unpkg.com/htmx-ext-sse@2.2.2/sse.js
```
...
```bash
git add packages/papan/src/ui/public packages/papan/src/ui/static.ts
git commit -m "feat(papan): vendor HTMX and add static asset route"
```
...
- Create: `packages/papan/src/ui/layout.ts`
- Create: `packages/papan/src/ui/partials/state-badge.ts`
- Create: `packages/papan/tests/ui/state-badge.test.ts`
...
```bash
git add packages/papan/src/ui/layout.ts packages/papan/src/ui/partials packages/papan/tests/ui
git commit -m "feat(papan): add UI layout shell and state-badge partial"
```
...
- Create: `packages/papan/src/ui/partials/issue-row.ts`
- Create: `packages/papan/src/ui/pages/list.ts`
- Create: `packages/papan/src/ui/routes.ts`
- Create: `packages/papan/tests/ui/issue-row.test.ts`
...
```bash
git add packages/papan/src/ui packages/papan/tests/ui/issue-row.test.ts
git commit -m "feat(papan): add issue list page"
```
...
- Create: `packages/papan/src/ui/pages/detail.ts`
- Create: `packages/papan/src/ui/partials/comment.ts`
- Create: `packages/papan/src/ui/partials/history-item.ts`
- Modify: `packages/papan/src/ui/routes.ts` — add `uiDetailRoute()`
- Create: `packages/papan/tests/ui/detail.test.ts`
...
```bash
git add packages/papan/src/ui packages/papan/tests/ui/detail.test.ts
git commit -m "feat(papan): add issue detail page with HTMX interactions"
```
...
- Create: `packages/papan/src/ui/pages/new.ts`
- Modify: `packages/papan/src/ui/routes.ts` — add `uiNewRoute()` (GET) and `uiCreatePostRoute()` (POST form handler)
- Create: `packages/papan/tests/ui/new.test.ts`
...
```bash
git add packages/papan/src/ui packages/papan/tests/ui/new.test.ts
git commit -m "feat(papan): add new-issue page with Linear URL paste"
```
...
- Modify: `packages/papan/src/index.ts`
- Create: `packages/papan/src/main.ts`
...
export function runPapan(opts: RunOptions = {}) {
  ...
  const apiToken = process.env["PAPAN_API_TOKEN"] || undefined;
...
```bash
PAPAN_DB_PATH=/tmp/papan-smoke.db bun run packages/papan/src/index.ts --port 3033 &
sleep 1
curl -s http://localhost:3033/ | head -c 200
kill %1 2>/dev/null || true
```

Expected: HTML containing `<title>Issues · papan</title>`.

- [ ] **Step 4: Commit**

```bash
git add packages/papan/src/index.ts packages/papan/src/main.ts
git commit -m "feat(papan): wire CLI entry and main bootstrap"
```
...
>
> - Server-rendered HTML, no SPA, no React. Output is a single hand-written CSS file at `packages/papan/src/ui/public/style.css`.
> - Existing HTML structure in `packages/papan/src/ui/pages/*` and `packages/papan/src/ui/partials/*` is the contract; do not change DOM structure or class names.
...
> Deliverables:
>
> 1. Replacement `packages/papan/src/ui/public/style.css`.
...
Start the server (`bun run packages/papan/src/index.ts --port 3033 --db /tmp/papan-design.db`), visit:
...
```bash
git add packages/papan/src/ui
git commit -m "feat(papan): apply frontend-design visual pass"
```
...
- Create: `packages/papan/tests/integration/full-cycle.test.ts`
...
Run: `RUN_INTEGRATION=1 bun test packages/papan/tests/integration`
Expected: PASS.
...
```bash
git add packages/papan/tests/integration
git commit -m "test(papan): add integration test for full HTTP cycle"
```
...
Run papan and verify the three dalang-facing endpoints respond correctly:

```bash
PAPAN_DB_PATH=/tmp/papan-contract.db bun run packages/papan/src/index.ts --port 3034 &
sleep 1
...
```bash
git add -A
git commit -m "chore(papan): post-implementation cleanup"
```
...
- ✅ **Spec coverage:** every section of the papan spec maps to at least one task. Phases 1–4 cover §3–§5 (stack, layout, data model, states). Phase 5 covers §7 API. Phase 6+7 cover §8 UI (with frontend-design skill explicitly invoked in Phase 7 per user request). Phase 8 covers §13 (DoD) + §12 integration test.
...
- ✅ **Scope:** single implementation plan; produces working, testable software (a runnable papan server passing all tests + manual smoke). Dalang work is a separate plan that consumes papan's REST contract.

---

## Next plan

After this plan is fully executed, proceed to `2026-04-29-dalang-orchestrator.md` (to be written separately). That plan starts from a state where the monorepo skeleton (Phase 0 here) and a runnable papan are already in place.
