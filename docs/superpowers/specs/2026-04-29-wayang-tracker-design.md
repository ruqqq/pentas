...
`wayang` is a single-user agent inbox. It is the issue tracker that feeds `dalang`, the orchestrator. Issues are copy-pasted into wayang from external sources (primarily company Linear cards assigned to the user); dalang polls wayang for issues in active states, runs Claude Code agents against them, and the agent updates state and posts progress comments via wayang's API.

Source-of-truth for the original Linear ticket stays at the company. wayang holds local copies plus their dalang-managed lifecycle. v1 has no sync-back to Linear.

Conformance: wayang implements the tracker contract from the dalang spec §11 verbatim (REST endpoints + `NormalizedIssue` shape) and adds the agent-mutation surface (PATCH/POST/DELETE) needed for the agent to drive its own workflow.
...
SQLite database at `~/.wayang/wayang.db` (override via `WAYANG_DB_PATH`).

```sql
CREATE TABLE issues (
  id              TEXT PRIMARY KEY,        -- ULID
  identifier      TEXT NOT NULL UNIQUE,    -- 'JUARA-1', monotonic
  title           TEXT NOT NULL,
  description     TEXT,
...
-- seq('issue_identifier', N) → next allocated identifier is 'JUARA-(N+1)'
```
...
Verbatim copy of dalang spec §11.1. wayang MUST satisfy these unchanged.
...
Single global stream; clients filter client-side. wayang's UI subscribes to keep the list and detail pages live while dalang's agent mutates issues.

### 7.4 Auth

- If `WAYANG_API_TOKEN` is set in the environment, every `/api/v1/*` request must carry `Authorization: Bearer <token>`. Missing/wrong → `401`.
- If `WAYANG_API_TOKEN` is unset, `/api/v1/*` is open. Server still binds `127.0.0.1` by default.
...
- Public `identifier`: `JUARA-N`, monotonic. Allocation:
  ...
  (initialized to `0` so the first issue is `JUARA-1`).
...
```
wayang [--port <n>] [--db <path>]
```

- Defaults:
  - port: `WAYANG_PORT` env or `3001`.
  - db: `WAYANG_DB_PATH` env or `~/.wayang/wayang.db`.
- `--port 0` requests an ephemeral port.
- Startup:
  1. Ensure `~/.wayang/` exists.
...
- Optional MCP server façade so the agent can call wayang via typed tool schemas instead of raw HTTP.
