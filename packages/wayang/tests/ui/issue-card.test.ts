...
describe("renderIssueCard", () => {
  ...
  test("state select includes all five states with current selected", () => {
    const html = renderIssueCard(issue);
    expect(html).toContain('<option value="Todo">Todo</option>');
    expect(html).toContain('<option value="In Progress" selected>In Progress</option>');
    expect(html).toContain('<option value="In Review">In Review</option>');
