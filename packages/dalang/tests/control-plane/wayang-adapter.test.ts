...
test("fetchDispatchableWork encodes states and paginates", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  const issues = await adapter.fetchDispatchableWork({ activeStates: ["Todo", "In Progress"], ownership: { mode: "none" } });
...
test("fetchDispatchableWork filters by label ownership", async () => {
  nextResponse = { status: 200, body: {
    issues: [
      { id: "i1", identifier: "X-1", title: "t1", state: "Todo", labels: ["Dalang"], blocked_by: [] },
      { id: "i2", identifier: "X-2", title: "t2", state: "Todo", labels: ["Other"], blocked_by: [] },
    ],
    next_cursor: null,
  }};
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
...
test("sends Authorization header when api_key is set", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: "secret-1" });
...
test("non-200 throws control-plane error control_plane_status_error", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  await expect(adapter.fetchDispatchableWork({ activeStates: ["Todo"], ownership: { mode: "none" } })).rejects.toMatchObject({
...
test("paginates across multiple pages preserving order", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: `http://localhost:${server.port}`, apiKey: null });
  const out = await adapter.fetchDispatchableWork({ activeStates: ["Todo"], ownership: { mode: "none" } });
...
test("addComment throws control-plane error(control_plane_write_error) on non-2xx", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  await expect(adapter.addComment("issue-1", "x")).rejects.toMatchObject({ code: "control_plane_write_error" });
});
...
test("updateState throws control-plane error(control_plane_write_error) on non-2xx", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  await expect(adapter.updateState("issue-1", "Bad")).rejects.toMatchObject({ code: "control_plane_write_error" });
});
...
test("listComments throws control-plane error(control_plane_status_error) on non-2xx", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  await expect(adapter.listComments("issue-1")).rejects.toMatchObject({ code: "control_plane_status_error" });
});
...
test("listComments throws control-plane error(control_plane_malformed_payload) when comments is not array", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  await expect(adapter.listComments("issue-1")).rejects.toMatchObject({ code: "control_plane_malformed_payload" });
});
...
test("listHistory throws control_plane_status_error on non-2xx", async () => {
  nextResponse = { status: 500, body: { error: "x" }};
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  await expect(adapter.listHistory("issue-1")).rejects.toMatchObject({ code: "control_plane_status_error" });
});
...
test("listHistory throws control_plane_malformed_payload when history is not array", async () => {
  ...
  const adapter = new WayangControlPlaneAdapter({ endpoint: baseURL(), apiKey: null });
  await expect(adapter.listHistory("issue-1")).rejects.toMatchObject({ code: "control_plane_malformed_payload" });
});
