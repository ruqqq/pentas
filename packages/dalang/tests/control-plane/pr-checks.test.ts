...
describe("summarise", () => {
  test("all pass → passed", () => {
    expect(summarise([{ name: "a", state: "S", bucket: "pass", link: "l" }]))
      .toEqual({ kind: "passed", failures: [] });
  });
...
describe("latestActionForSha", () => {
  ...
  test("matches full-length sha against short-sha tag in comment", () => {
    ...
    const comments: TrackerComment[] = [
      { id: "1", author: "agent", body: "[pr_checks_failed] sha=abc1234 attempt=1/3", created_at: "2026-01-01T00:00:00Z" },
    ];
...
describe("decideAction", () => {
  ...
  test("no PR resolved → no_pr_bounce", () => {
    expect(decideAction({ ...base, prResolved: null, comments: [], summary: null }))
      .toEqual({ kind: "no_pr_bounce" });
  });
  ...
  test("rerun_flakes=false → bounce immediately on first failure for sha", () => {
    expect(decideAction({
      budget: 3, rerunFlakes: false,
      prResolved: { sha: "abc" }, comments: [],
      summary: { kind: "failed", failures: [{ name: "x", state: "F", bucket: "fail", link: "l" }] },
    })).toMatchObject({ kind: "failed_bounce", attempt: 1 });
  });
  ...
  test("dedupes correctly when prResolved.sha is a full git sha", () => {
    const fullSha = "abc1234567890abcdef0123456789012345678901";
    const comments = [
      { id: "1", author: "agent", body: "[pr_checks_passed] sha=abc1234", created_at: "2026-01-01T00:00:00Z" },
    ];
    expect(decideAction({ budget: 3, rerunFlakes: true,
      prResolved: { sha: fullSha }, comments,
