...
const runQueryFactory = () => async function* () {
...
test("uses explicit wayang control_plane endpoint and api_key in bootstrap adapter", async () => {
  ...
  const path = join(dir, "WORKFLOW.md");
  const endpoint = `http://127.0.0.1:${wayang.port}`;
