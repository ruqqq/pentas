...
export class GithubProjectsControlPlaneAdapter implements ControlPlaneAdapter {
    ...
    for (const state of requiredStates) {
      if (!meta.statusOptions.has(state.toLowerCase())) {
        throw new ControlPlaneError(
          ...
          `github project status option not found: ${state}`,
        );
      }
    }
  ...
  async fetchDispatchableWork(query: DispatchQuery): Promise<WorkItem[]> {
    ...
    return all.flatMap((raw) => {
      ...
      return item && query.activeStates.some((s) => s.toLowerCase() === item.state.toLowerCase())
        ? [item]
        : [];
    });
  }
    ...
    for (let page = 1; ; page += 1) {
      ...
      if (!Array.isArray(batch)) {
        throw new ControlPlaneError(
          ...
          "github issue comments: expected array",
        );
      }
      comments.push(...batch);
  ...
  async addComment(
    ...
    const { number, repository } = await this.issueRefForProjectItem(workItemId);
    const [owner, repo] = splitRepository(repository);
    await this.client.restJson(`/repos/${owner}/${repo}/issues/${number}/comments`, "POST", {
      body,
    });
  }
  ...
  private async resolveMetadata(): Promise<GithubProjectMetadata> {
    ...
    if (!statusField || typeof statusField.id !== "string") {
      throw new ControlPlaneError(
        "control_plane_validation_error",
        `github project status field not found: ${this.cfg.statusField}`,
      );
    }

    const statusOptions = optionsByLowerName(statusField.options);
    const ownershipFieldName =
      this.cfg.ownership.mode === "project_field" ? this.cfg.ownership.field : null;
    const branchFieldId = fieldId(fields, this.cfg.branchField);
    if (this.cfg.branchField && !branchFieldId) {
      throw new ControlPlaneError(
        "control_plane_validation_error",
        `github project branch field not found: ${this.cfg.branchField}`,
      );
    }
    ...
    if (ownershipFieldName && !ownershipFieldId) {
      throw new ControlPlaneError(
        "control_plane_validation_error",
        `github project ownership field not found: ${ownershipFieldName}`,
      );
    }
    const ownershipOptions = ownershipFieldName
      ...
      : new Map();
    if (
      this.cfg.ownership.mode === "project_field" &&
      !ownershipOptions.has(this.cfg.ownership.value.toLowerCase())
    ) {
      throw new ControlPlaneError(
        "control_plane_validation_error",
        `github project ownership option not found: ${this.cfg.ownership.value}`,
      );
    }
    this.metadata = {
      projectId,
  ...
  private async fetchProjectFields(): Promise<{
    projectId: string;
    fields: Array<Record<string, unknown>>;
  }> {
    const query =
      this.cfg.ownerType === "organization" ? PROJECT_METADATA_QUERY : USER_PROJECT_METADATA_QUERY;
    ...
    do {
      ...
      if (typeof projectObj.id !== "string") {
        throw new ControlPlaneError(
          "control_plane_malformed_payload",
          "github project metadata missing id",
        );
      }
      projectId = projectObj.id;
      const fieldConnection =
        projectObj.fields && typeof projectObj.fields === "object"
          ? (projectObj.fields as Record<string, unknown>)
          : null;
      fields.push(
        ...nodes(fieldConnection).filter(
          (x): x is Record<string, unknown> => x !== null && typeof x === "object",
        ),
      );
      const pageInfo =
        fieldConnection?.pageInfo && typeof fieldConnection.pageInfo === "object"
      ...
      if (fieldCursor === null) {
        throw new ControlPlaneError(
          ...
          "github project fields missing next cursor",
        );
      }
    } while (fieldCursor);
    if (!projectId)
  ...
  private async fetchProjectItems(): Promise<unknown[]> {
    ...
    do {
      const data: { node?: unknown } = await this.client.graphql(PROJECT_ITEMS_QUERY, {
        projectId: meta.projectId,
        cursor,
      });
      const project: { items?: unknown } | null =
        data.node && typeof data.node === "object" ? (data.node as { items?: unknown }) : null;
      const items: Record<string, unknown> | null =
        project?.items && typeof project.items === "object"
          ? (project.items as Record<string, unknown>)
          : null;
      out.push(...nodes(items));
      const pageInfo: { hasNextPage?: unknown; endCursor?: unknown } | null =
        items?.pageInfo && typeof items.pageInfo === "object"
      ...
      if (pageInfo.hasNextPage === true && cursor === null) {
        throw new ControlPlaneError(
          "control_plane_missing_pagination_cursor",
          "github project items missing next cursor",
        );
      }
  ...
  private async issueRefForProjectItem(
    itemId: string,
  ): Promise<{ number: number; repository: string }> {
    const data = await this.client.graphql<{ node?: unknown }>(PROJECT_ITEM_CONTENT_QUERY, {
      itemId,
    });
    const item =
      data.node && typeof data.node === "object" ? (data.node as { content?: unknown }) : null;
    const content =
    ...
    if (!content || content.__typename !== "Issue" || typeof content.number !== "number") {
      throw new ControlPlaneError(
        "control_plane_validation_error",
        `github project item is not an issue: ${itemId}`,
      );
    }
    return {
  ...
  private async resolvePullRequest(item: WorkItem): Promise<GithubPullRequestRef | null> {
    ...
    const [owner, repo] = this.repoParts();
    const prs = await this.client.restJson<
      Array<{
        number: number;
        html_url: string;
        node_id?: string;
    ...
    const pr = prs[0];
    return pr
      ? { number: pr.number, url: pr.html_url, sha: pr.head.sha, nodeId: pr.node_id ?? null }
      : null;
  }
  ...
  private async fetchChecks(pr: GithubPullRequestRef): Promise<GithubCheck[]> {
    ...
    const [owner, repo] = this.repoParts();
    const prData = await this.client.restJson<{ head: { sha: string } }>(
    ...
    }>(`/repos/${owner}/${repo}/commits/${prData.head.sha}/check-runs`, "GET");
  ...
  private async fetchStatusCheckRollup(pr: GithubPullRequestRef): Promise<GithubCheck[]> {
    ...
    do {
      ...
      const prNode =
        data.node && typeof data.node === "object" ? (data.node as { commits?: unknown }) : null;
      const commitNode = firstNode(prNode?.commits);
      const commitContainer =
        commitNode && typeof commitNode === "object" ? (commitNode as { commit?: unknown }) : null;
      const commit =
        commitContainer?.commit && typeof commitContainer.commit === "object"
          ? (commitContainer.commit as { statusCheckRollup?: unknown })
          : null;
      const rollup =
        commit?.statusCheckRollup && typeof commit.statusCheckRollup === "object"
          ? (commit.statusCheckRollup as { contexts?: unknown })
          : null;
      const contexts =
        rollup?.contexts && typeof rollup.contexts === "object"
      ...
      for (const raw of nodes(contexts)) {
        ...
        if (c.__typename === "CheckRun") {
          const checkSuite =
            c.checkSuite && typeof c.checkSuite === "object"
              ? (c.checkSuite as { workflowRun?: unknown })
              : null;
          const workflowRun =
            checkSuite?.workflowRun && typeof checkSuite.workflowRun === "object"
      ...
      const pageInfo =
        contexts?.pageInfo && typeof contexts.pageInfo === "object"
      ...
      if (cursor === null) {
        throw new ControlPlaneError(
          "control_plane_missing_pagination_cursor",
          "github status check rollup missing next cursor",
        );
      }
    } while (cursor);
    return out;
  ...
  private async rerunFailedChecks(
    _pr: GithubPullRequestRef,
    checks: GithubCheck[],
  ): Promise<number> {
    const [owner, repo] = this.repoParts();
    let count = 0;
    ...
    for (const check of checks) {
      ...
      await this.client.restJson(
        `/repos/${owner}/${repo}/actions/runs/${check.runId}/rerun-failed-jobs`,
        "POST",
        {},
      );
      count += 1;
  ...
  for (const option of raw) {
    if (option && typeof option === "object") {
      ...
      if (typeof o.id === "string" && typeof o.name === "string")
        out.set(o.name.toLowerCase(), o.id);
    }
  }
  return out;
}
...
function itemRepositoryNameWithOwner(raw: unknown): string | null {
  ...
  return content && typeof content === "object"
    ? repositoryNameWithOwner(content as Record<string, unknown>)
    : null;
}
...
function repositoryNameWithOwner(content: Record<string, unknown>): string | null {
  ...
  if (
    ...
    typeof (repository as { nameWithOwner?: unknown }).nameWithOwner === "string"
  ) {
...
function statusContextBucket(state: unknown): GithubCheck["bucket"] {
  ...
  if (state === "PENDING" || state === "pending" || state === "EXPECTED" || state === "expected")
    return "pending";
