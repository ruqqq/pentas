...
6. Move the item to `Waiting PR Checks`, then end this turn. The control plane PR-check reconciler will move it to `Ready for Human Review` on pass, back to `In Dev` on failure, or to `Ready for Human Review` after repeated failure-budget exhaustion.
